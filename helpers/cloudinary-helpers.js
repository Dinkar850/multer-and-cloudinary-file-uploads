...
const uploadToCloudinary = async (filePath) => {
  try {
    ...
    return {
      url: result.secure_url, //for storing in our mongodb
      publicId: result.public_id, //for also deleting from cloudinary when we do from mongo
    };
  } catch (e) {
    console.error("Error while uploading to cloudinary", error);
    throw new Error("Error while uploading to cloudinary"); //why?
  }
};
